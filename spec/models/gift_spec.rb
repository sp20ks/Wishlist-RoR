...
RSpec.describe Gift, type: :model do
  it 'successfully create gift' do
    ...
    a = Gift.new(name: 'name', link: 'link', user_id: test_user.id, description: 'desc',
                 wishlist_type_id: wishlist_type.id)
  ...
  it 'successfully delete gift' do
    test_user = User.create(f_name: 'Имя', l_name: 'Фамилия', login: 'login1', email: 'example_test@ya.ru',
                            password: 'Qwerty12', password_confirmation: 'Qwerty12')
    wishlist_type = WishlistType.create(name: 'test')
    a = Gift.create(name: 'name', link: 'link', user_id: test_user.id, description: 'desc',
                    wishlist_type_id: wishlist_type.id)
    a.destroy
